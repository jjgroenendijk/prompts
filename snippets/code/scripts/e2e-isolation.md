...
Point the environment variable the script reads for its project root at the temp directory, seed that directory with the fixture data the run needs, then launch the script as a subprocess with that environment passed through. Assert against files inside the temp tree and delete it when the test ends. This allows testing scripts in isolation without affecting the main project.
